...
def Detect3x(z, b, clock, reset):
    ...
    # next_state.next[1] = ...
        next_state.next[1] = (~state[1] & state[0] & ~b) | (state[1]
        & ~state[0] & b)
        # next_state.next[0] = ...
        next_state.next[0] = (~state[1] & ~state[0] & b) | (state[1]
        & ~state[0] & ~b) | (state[1] & state[0] & b)

    # generate output
    @always_comb
    def output_logic():
        # generate z from state and b
        z.next = (state * 2 + b % 3 == 0)
...
if __name__ == "__main__":
    ...
    parser.add_argument('bits', nargs='?', default="111000101", help='bits to be shifted in')
